# -*- coding: utf-8 -*-
"""
Created on Thu Sep 24 16:17:13 2017
...
#print(countyLoc)
#print(type(countyLoc), type(OP_Sum_tidyData))
#print(OP_Sum_tidyData)
...
for stateID in OP_Sum_tidyData[2017]:
    for countyID in OP_Sum_tidyData[2017][stateID]:
        ...
        if str(countyLoc[stateID][countyID]['lat']) == '{}':
            ...
            Z.append(OP_Sum_tidyData[2017][stateID][countyID])
...
plt.title('The Number of Every State\'s Opioids Reports in 2017')
'''
...
plt.savefig('map2017.jpg', dpi=300)
...
'''
for keys in countyLoc.keys():
    print(keys)

cFile = open("loc.csv", "r")
reader = csv.DictReader(cFile)

print(reader.fieldnames)    

for row in reader:
    if row['state_id'] in states.keys():
        if row['county_fips'] not in countyLoc.keys():
            countyLoc[row['state_id']][row['county_fips']]['lat'] = row['lat']
            countyLoc[row['state_id']][row['county_fips']]['lng'] = row['lng']
            countyLoc[row['state_id']][row['county_fips']]['name'] = row['county_name']
        else:
            countyLoc[row['state_id']][row['county_fips']]['lat'] = (row['lat'] + countyLoc[row['county_fips']]['lat']) / 2
            countyLoc[row['state_id']][row['county_fips']]['lng'] = (row['lng'] + countyLoc[row['county_fips']]['lng']) / 2 

for key in states.keys():
    print(countyLoc[key])
    
np.save("countyLoc.npy", countyLoc)

'''
#locInf = pd.read_csv('loc.csv')
#print(locInf)
#print(type(locInf))
#print(type(locInf[2:3]))
