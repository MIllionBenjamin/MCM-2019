...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 28 02:03:08 2019
...
for opID in OP_Sort_Data['Unsorted'].keys():
    for year in OP_Sort_Data['Unsorted'][opID].keys():
        for stateID in OP_Sort_Data['Unsorted'][opID][year].keys():
            for countyID in OP_Sort_Data['Unsorted'][opID][year][stateID].keys():
                ...
                if str(countyLoc[stateID][countyID]['lat']) == '{}':
                    ...
                    Z[alreadyCounty.index(countyID)] += OP_Sort_Data['Unsorted'][opID][year][stateID][countyID]
                else:
                    ...
                    Z.append(OP_Sort_Data['Unsorted'][opID][year][stateID][countyID])
...
print(Z.min())
print(Z.max())
colours = plt.cm.rainbow(norm(Z + 50)) 

# 具体函数方法可用 help(function) 查看，如：help(ax.plot_surface)
ax.bar3d(Y, X, 0, dx = 0.5, dy = 0.5, dz = Z, color=colours)

plt.title('The Number of 8-year Opioids Reports of Unsorted Opioids')

colourMap = plt.cm.ScalarMappable(cmap=plt.cm.rainbow)
colourMap.set_array(Z - 50)
...
plt.savefig('mapUnsorted_opioids.jpg', dpi=300)
